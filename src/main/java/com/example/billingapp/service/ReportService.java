...
public class ReportService {
    ...
    public ReportService(TagihanRepository tagihanRepository, EmailService emailService,
                         AppSettingsRepository appSettingsRepository, UserRepository userRepository) {
    ...
    public void sendMonthlyReport(int month, int year) {
        List<Area> areas = Arrays.asList(Area.values());
        ...
        for (Area area : areas) {
            ...
            if (tagihanAreaIni.isEmpty()) {
                System.out.println("INFO: Tidak ada tagihan untuk area " + area.getDisplayName() + ". Email tidak dikirim.");
                continue;
            }
            ...
            try {
                ...
                String subject = "Laporan Pembayaran Area " + area.getDisplayName() + " - " + bulan + " " + tahun;
                ...
                System.out.println("✅ Laporan untuk area " + area.getDisplayName() + " berhasil dikirim ke: " + String.join(", ", recipients));

            } catch (Exception e) {
                System.err.println("❌ GAGAL mengirim laporan untuk area " + area.getDisplayName() + ": " + e.getMessage());
            }
        }
    }
    ...
    private File createPdfForArea(List<Tagihan> tagihans, Area area, String bulan, String tahun, List<File> attachments) throws DocumentException, IOException {
        ...
        File pdfFile = new File(dir, "laporan_" + area.name() + "_" + bulan + "_" + tahun + ".pdf");
        ...
        Paragraph title = new Paragraph("Laporan Tagihan Area " + area.getDisplayName() + " - " + bulan + " " + tahun, titleFont);
    ...
    table.addCell(new PdfPCell(new Phrase("Bukti Bayar", headFont)));
        ...
        for (Tagihan t : tagihans) {
           ...
           if ("Belum Dibayar".equalsIgnoreCase(t.getStatus())) {
            ...
            String buktiPath = (pembayaran != null) ? pembayaran.getBuktiTransferPath() : null;
            ...
            if (StringUtils.hasText(buktiPath)) {
                ...
                // ✅ LOGIKA KUNCI: Menambahkan file bukti bayar ke daftar lampiran
        ...
        // Tambahkan file PDF itu sendiri ke daftar lampiran
        attachments.add(pdfFile);
    ...
    private String createHtmlBodyForArea(List<Tagihan> tagihans, Area area, String bulan, String tahun) {
        ...
        htmlBody.append("<p>Berikut adalah ringkasan Laporan Pembayaran untuk area <strong>")
                .append(area.getDisplayName()).append("</strong> bulan ").append(bulan).append(" ").append(tahun).append(".</p>");
